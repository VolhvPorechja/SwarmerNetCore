...
namespace Swarmer.AM.DAL.Repositories
{
    public class UsersRepository : RepositoryBase, UsersRepositoryContract
    {
        ...
        public void AddAuthenticationData(Guid userId, string type, string secret)
        {
            var now = DateTime.Now;
            mConnection.Execute(
                $"INSERT INTO Entries(id,created,updated,userId,type,secret)" +
                $"values(@id,@created,@updated,@userid,@type,@secret)",
                new
                {
                    id = Guid.NewGuid(),
                    created = now,
        ...
        public AuthneticationData GetAuthenticationData(Guid userId, string type)
        {
            return
                mConnection.QueryFirstOrDefault<AuthneticationData>(
                    $"SELECT * FROM Entries WHERE userid = @userid and type = @type",
                    new
                    {
                        userId,
                        type
